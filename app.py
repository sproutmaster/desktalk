...
def create_ticket():
    ...
    owner_token = request.form.get('token')
    owner_name = user_table.find_one({'_id': ObjectId(owner_token)})['user']
    query['owner_name'] = owner_name
    tid = ticket_table.insert_one(query).inserted_id
    query['ticket_id'] = str(tid)
    # update both agent and customer
    socketio.emit('ticket_update', {'action': 'new', 'owner_name': owner_name, 'owner': owner_token}, broadcast=True)
...
def solved():
    agent_token = request.form.get('token')
    ticket_id = request.form.get('ticket_id')
    owner_token = ticket_table.find_one({'_id': ObjectId(ticket_id)})['owner']
    owner_name = user_table.find_one({'_id': ObjectId(owner_token)})['user']
    agent_name = user_table.find_one({'_id': ObjectId(agent_token)})['user']
    status = request.form.get('status')
    to_put = {'status': status}
    if request.form.get('status') == 'closed':
        to_put['closed_by'] = agent_name
        to_put['closed_at'] = int(time())

    ticket_table.update_one({'_id': ObjectId(ticket_id)}, {'$set': to_put})

    socketio.emit('ticket_update', {'action': 'update', 'owner_name': owner_name, 'owner': owner_token,
                                    'updated_by': agent_token, 'status': request.form.get('status'),
                                    'updated_by_name': agent_name}, broadcast=True)
...
def get_talk_link():
    ...
    ticket_owner_token = ticket_table.find_one({'_id': ObjectId(ticket_id)})['owner']
    ticket_owner_name = user_table.find_one({'_id': ObjectId(ticket_owner_token)})['user']

    socketio.emit('ticket_update', {'action': 'update', 'owner_name': ticket_owner_name, 'owner': ticket_owner_token,
                                    'updated_by': agent_token, 'status': 'talk generated',
                                    'updated_by_name': agent_name}, broadcast=True)

    ticket_table.update_one({'_id': ObjectId(request.form.get('ticket_id'))},
                            {'$set': {'join_link': link, 'taken_by': agent_name}})
    return {'link': link}
